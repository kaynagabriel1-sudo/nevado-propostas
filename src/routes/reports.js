...
router.get('/monthly', requireAuth, async (req, res) => {
  ...
  let base = `FROM proposals p JOIN users u ON p.seller_id = u.id
              WHERE SUBSTRING(p.created_at, 6, 2) = '${m}' 
              AND SUBSTRING(p.created_at, 1, 4) = '${y}'`;
  ...
  try {
    ...
    const bySeller = await db.prepare(`
      ...
      ${base} GROUP BY u.id, u.name, u.color, u.goal ORDER BY volume DESC`).all();
